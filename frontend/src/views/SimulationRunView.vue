  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close simulation environment, trying force stop...`)
        try {
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh does not show fullscreen loading to avoid flicker
...
const startGraphRefresh = () => {
  ...
  addLog('Enabled graph real-time refresh (30s)')
...
onMounted(() => {
  ...
  // Log maxRounds config (value already retrieved from query params at initialization)
  if (maxRounds.value) {
