...
const handleNextStep = (params = {}) => {
  ...
  // If custom rounds exist, pass via query params
  if (params.maxRounds) {
 ...
 * When user returns from Step 3 to Step 2, assume user wants to exit the simulation
 */
const checkAndStopRunningSimulation = async () => {
  if (!currentSimulationId.value) return
  
  try {
    // First check if the simulation environment is alive
    const envStatusRes = await getEnvStatus({ simulation_id: currentSimulationId.value })
    
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Detected running simulation environment, shutting down...')
      
      // Attempt graceful shutdown of simulation environment
      try {
