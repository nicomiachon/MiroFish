    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <template v-if="graphData">
              ...
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} Relationships</span>
              <span class="stat-divider">|</span>
            </template>
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (displayed whenever data is available) -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyzes the content and automatically generates an ontology structure suitable for opinion simulation (entity types + relationship types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on the generated ontology, documents are chunked and the Zep API is called to build the knowledge graph, extracting entities and relationships
                </div>
              </div>
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling for building tasks
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Failed to start graph building'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Failed to start graph building: ' + (err.message || 'Unknown error')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // First get project info to retrieve graph_id
...
const renderGraph = () => {
  ...
  // Create node map for name lookups
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Draw edges (with clickable transparent wide line)
  ...
  simulation.on('tick', () => {
    // Update all edge positions (including visible and transparent click areas)
