            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation complete, but the entire section may not be finished yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep the button at the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-questions - full extraction, no limit on quantity
    ...
    // Extract key facts - full extraction, no limit on quantity
    ...
    // Extract relationship chains - full extraction, no limit on quantity
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - full extraction, no limit on quantity
    ...
    // Extract historical/expired facts - full extraction, no limit on quantity
    ...
    // Extract involved entities - full extraction, no limit on quantity
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview subject selection reasons
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **alumni_345(index=1)**: As a university alumnus...
        ...
        if (name) {
          ...
          // Continuation of reason text (exclude ending summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g. "student", "education practitioner", etc.)
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question has "1." prefix, special handling is needed
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible: when only one platform tag exists)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer exists, copy as default display only when not placeholder text
          ...
          // No platform tags (very old format), use entire text as answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - full extraction, no limit on quantity
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No current active memories'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Try to split answers by question numbers
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 2. "1. " or "\n1. " (number+dot, old format backward compatible)
      let matches = []
      let match

      // Prefer "问题X：" format first
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // If no numbering found or only one, return the whole text
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for first question, empty for the rest
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder text) and content differs
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (only shown when real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), as section title is already shown in the outer layer
  ...
  // Clean up <br> between block-level elements and following paragraph start tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: maintain incrementing numbers when single-item <ol> are separated by paragraph content
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task is complete, scroll to top; otherwise scroll to bottom to follow latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after 最终答案: (Chinese "Final Answer")
  ...
  // If content is long and contains markdown format, try to remove thinking process and return
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: prefixed thinking process
