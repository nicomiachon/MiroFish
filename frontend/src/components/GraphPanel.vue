...
const wasSimulating = ref(false) // Track whether simulation was running
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to non-simulating, show finished hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges distribute in the same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster independent node groups toward center
  ...
  // Links - use path for curve support
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset from link direction, proportional to distance for visible curves
    // More edges means a larger offset ratio
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actual dragging started (movement exceeds threshold)
      ...
      .on('end', (event, d) => {
        // Only let simulation gradually stop if actual dragging occurred
        if (d._isDragging) {
