<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">Prediction Records</span>
      <div class="section-line"></div>
    </div>
...
let pendingState = null  // Pending target state to execute
...
const containerStyle = computed(() => {
  ...
  // Calculate required height: rows * card height + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count for current row, ensure centering
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record latest regardless of animation state)
        ...
        // If animating, just record state and process after animation ends
        ...
        // Use debounce to delay state switch, prevent rapid flickering
        // Short delay for expand (50ms), longer delay for collapse (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly before executing to avoid rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upward, requires more scrolling to trigger expand
...
.grid-pattern {
  ...
  /* Position from top-left; height changes only extend at bottom, not affecting existing grid positions */
