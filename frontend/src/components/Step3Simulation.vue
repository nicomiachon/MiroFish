...
const phase = ref(0) // 0: Not Started, 1: In Progress, 2: Completed
...
const doStartSimulation = async () => {
  ...
  // Reset all state first, ensuring no residual effects from previous simulation
  resetAllState()
...
// Track previous round number for each platform to detect changes and output logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation is complete (via runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Extra check: if backend hasn't updated runner_status yet but platforms report complete
      // Determine via twitter_completed and reddit_completed status
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform is done, check if all enabled platforms are done
