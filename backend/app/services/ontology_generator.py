...
Interface: Analyzes text content and generates entity and relationship type definitions suitable for social simulation.
"""
...
- Entities can influence each other, repost, comment, and respond to one another
...
- Representatives of specific groups (e.g., alumni associations, fan communities, advocacy groups, etc.)
...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. When a person does not belong to any other more specific person type, assign them here.
   - `Organization`: Fallback type for any organization or institution. When an organization does not belong to any other more specific organization type, assign them here.
...
- Various people will appear in the text, such as "a school teacher", "a passerby", "some internet user"
- If no specific type matches, they should be assigned to `Person`
...
- Celebrity: A celebrity or influencer
...
4. All entity types must be real-world subjects that can post on social media; abstract concepts are not allowed
