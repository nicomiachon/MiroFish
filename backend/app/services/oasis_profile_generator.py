...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search API, so edges and nodes are searched
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determine whether the entity is an individual type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine whether the entity is a group/organization type"""
    ...
    def _generate_profile_with_llm(
        ...
        - Group/organization entity: generates a representative account profile
        """
        ...
        # Attempt multiple generations until success or max retries reached
        max_attempts = 3
        last_error = None
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if no comma or closing bracket after the last quote, the string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user profiles. Generate detailed, realistic personas for public opinion simulation, faithfully reproducing existing real-world circumstances as much as possible. You must return valid JSON format, and all string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        """Build detailed persona prompt for an individual entity"""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        return f"""Generate a detailed social media user persona for the entity, faithfully reproducing existing real-world circumstances as much as possible.
...
2. persona: Detailed persona description (2000 words of plain text), which should include:
   - Basic information (age, profession, educational background, location)
   - Personal background (significant experiences, connections to events, social relationships)
   ...
   - Personal memories (an important part of the persona: describe this individual's connection to events, and their existing actions and reactions within those events)
...
6. country: Country (in English, e.g., "France", "United States")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed persona prompt for a group/organization entity"""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        return f"""Generate a detailed social media account profile for an organization/group entity, faithfully reproducing existing real-world circumstances as much as possible.
...
2. persona: Detailed account profile description (2000 words of plain text), which should include:
   - Organization basic information (official name, organization type, founding background, main functions)
   ...
   - Stances and attitudes (official positions on key topics, approach to handling controversies)
   - Special notes (profile of the represented group, operational habits)
   - Organizational memory (an important part of the persona: describe this organization's connection to events, and its existing actions and reactions within those events)
...
- Organizational account communications must align with its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        completed_count = [0]  # Use list so it can be modified inside closures
        lock = Lock()
    ...
    def save_profiles(
        ...
        """
        Save profiles to file (choosing the correct format based on platform).
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        """
        Normalize gender field to the English format required by OASIS.
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Must include the user_id field -- this is the key for OASIS agent_graph.get_agent() matching!
